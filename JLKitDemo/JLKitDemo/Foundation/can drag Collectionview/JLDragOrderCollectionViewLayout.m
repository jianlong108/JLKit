//
//  JLDragOrderCollectionViewLayout.m
//  JLKitDemo
//
...
@interface JLDragOrderCollectionViewLayout ()
...
@implementation JLDragOrderCollectionViewLayout
...
- (void)moveItemIfNeeded {
    ...
    [self.collectionView performBatchUpdates:^{
        ...
        [_cellFakeView changeBoundsIfNeeded:CGRectMake(0, 0, attribute.frame.size.width, attribute.frame.size.height)];
    ...
    if (_cellFakeView != nil) {
        switch (pan.state) {
            case UIGestureRecognizerStateChanged:{
                ...
                {
                    case SupportDragOrientationAll:
                    {
                        //不限制拖动方向
                        center.x = self.fakeCellCenter.x + self.panTranslation.x;
                        center.y = self.fakeCellCenter.y + self.panTranslation.y;
                        ...
                        break;
                    case SupportDragOrientationHorizontal:
                    {
                        //只能横向拖动
                        center.x = self.fakeCellCenter.x + self.panTranslation.x;
                        ...
                        break;
                    case SupportDragOrientationVertical:
                    {
                        //只能纵向拖动
...
- (void)setDelegate:(id<JLDragOrderCollectionViewLayoutDelegate>)delegate{
    _delegate = delegate;
    self.collectionView.delegate = delegate;
}

- (void)setDataSource:(id<JLDragOrderCollectionViewLayoutDataSource>)dataSource{
...
- (instancetype)initWithCell:(UICollectionViewCell *)cell{
    ...
    if (self) {
        ...
        self.cellFakeHightedView.image = screenshot(cell);
        cell.highlighted = NO;
        self.cellFakeImageView.image = screenshot(cell);
